...
var Marked = React.createClass({displayName: 'Marked',
  render: function () {
    return React.DOM.div( {className:"Marked"}, ReactParser.parse(marked.Lexer.lex(this.props.markdown)));
  }
});
...
extend(ReactRenderer.prototype, {
  ...
  codespan: function (content) {
    return codespan( {key:itemsRenderedCount++}, content);
  },
...
function ReactParser() {
  this.tokens = [];
  this.token = null;
  this.options = { renderer: new ReactRenderer() };
  this.renderer = this.options.renderer;
}

ReactParser.parse = function (src) {
